...
![National Parks Service Regions - This poster explains the region categories used in this data set.](~/Documents/GitHub/National_Parks_Project/Data%20Sources)
...
base_map <- ggplot() +
  ...
  theme_minimal() +
  labs(title = "Total Visitation for Western United States National Parks")
...
all_routes$distance_category <- cut(
  ...
  breaks = c(0, 10, 50, 100, 200, Inf), # Customize the breaks as needed
  labels = c("0-10 miles", "10-50 miles", "50-100 miles", "100-200 miles", "200+ miles")
)

# Plot with color gradient based on distance
p <- base_map +
  geom_sf(data = route_sf, aes(color = total_distance_miles), size = 0.2) +
  geom_point(aes(x = start["lon"], y = start["lat"]), color = "red", size = 1, shape = 17) +
  geom_point(data = top_parks_merged, aes(x = Longitude, y = Latitude), color = "green", size = 1, shape = 17) +
...
p7 <- base_map +
  ...
  scale_color_brewer(palette = "Set1") +  # Qualitative color palette for clusters
...
p7 <- base_map +
  geom_sf(data = top_parks_cluster_sf, aes(color = factor(cluster)), size = 1.5, alpha = 0.9) +
  scale_color_brewer(palette = "Set1") +  # Qualitative color palette for clusters
  labs(title = "Cluster Analysis of National Parks", 
       color = "Cluster") +
  theme_minimal() +
...
Better! Now I want to know what the most efficient way to drive from park to park is so I will use the Traveling Salesman Problem (TSP). The TSP finds shortest possible route that visits each city exactly once and returns to the original city.
...
# Plot with arrows at midpoints
p8 <- base_map +
  geom_sf(data = routes_sf_lines, color = "black", size = 2, linetype = "solid", arrow = arrow(type = "open", length = unit(0.03, "inches"))) +
  geom_sf(data = routes_sf_lines, aes(color = as.factor(cluster)), size = .5, linetype = "solid", arrow = arrow(type = "open", length = unit(0.02, "inches"))) +
...
# Customize the plot
p <- base_map +
  geom_sf(data = routes_subcluster_sf_lines, color = "black", size = 2, linetype = "solid",
          arrow = arrow(type = "open", length = unit(0.05, "inches"))) +
  geom_sf(data = routes_subcluster_sf_lines, aes(color = as.factor(cluster)), size = 0.7, linetype = "solid",
          arrow = arrow(type = "open", length = unit(0.04, "inches"))) +
  scale_color_viridis_d(option = "plasma", direction = -1) +
  labs(title = "Optimized Routes for visitng parks Regional Parks",
       subtitle = "",
       color = "Cluster") +
